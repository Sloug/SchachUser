...
RUN cd /SchachUser && git pull && git checkout -t origin/mongoDB && sbt compile
ENTRYPOINT service mysql start && service mongod start && cd /SchachUser && PASSWORD=$(cat /etc/mysql/debian.cnf | grep "password"| head -1 | sed -n -e 's/^.*= //p') sbt run
